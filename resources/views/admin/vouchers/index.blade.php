...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
    ...
    <div class="flex items-center gap-4 w-full sm:w-auto">
        <a href="{{ route('admin.vouchers.create') }}" class="bg-pink-600 hover:bg-pink-700 text-white font-medium py-2 px-4 rounded-lg transition-colors duration-200 flex items-center justify-center w-full sm:w-auto text-sm sm:text-base">
...
<div class="mb-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
    ...
    <div class="bg-white rounded-lg shadow-md border border-gray-200 p-4 sm:p-6">
        <div class="flex items-center">
            ...
            <div>
                ...
                <p class="text-lg sm:text-2xl font-bold text-gray-800">Rp {{ number_format($statistics['total_discount'], 0, ',', '.') }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-md border border-gray-200 p-4 sm:p-6">
        <div class="flex items-center">
            <div class="w-10 h-10 sm:w-12 sm:h-12 bg-orange-100 rounded-lg flex items-center justify-center mr-3 sm:mr-4">
...
<div class="bg-white rounded-lg shadow-md border border-gray-200 p-4 sm:p-6 mb-8">
    ...
    <form method="GET" action="{{ route('admin.vouchers.index') }}" class="flex flex-col md:flex-row gap-4">
        ...
        <div class="w-full md:w-48">
            <select name="status" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500 focus:border-transparent text-sm sm:text-base">
        ...
        <!-- ANCHOR: Type Filter -->
        <div class="w-full md:w-48">
            <select name="type" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500 focus:border-transparent text-sm sm:text-base">
        ...
        <div class="w-full md:w-48">
            <select name="period" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500 focus:border-transparent text-sm sm:text-base">
        ...
        <!-- ANCHOR: Usage Filter -->
        <div class="w-full md:w-48">
            <select name="usage" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500 focus:border-transparent text-sm sm:text-base">
        ...
        <div class="flex flex-col sm:flex-row gap-2 sm:gap-0">
            <button type="submit" class="bg-gray-600 hover:bg-gray-700 text-white py-2 px-4 rounded-lg transition-colors duration-200 text-sm sm:text-base">
                Filter
            </button>
            @if(request('search') || request('status') || request('type') || request('period') || request('usage'))
                <a href="{{ route('admin.vouchers.index') }}" class="bg-red-600 hover:bg-red-700 text-white py-2 px-4 rounded-lg transition-colors duration-200 flex items-center justify-center text-sm sm:text-base">
                    Reset
                </a>
            @endif
        </div>
    ...
    <div class="overflow-x-auto">
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50 border-b border-gray-200">
                <tr>
                    <th class="px-4 sm:px-6 py-3 sm:py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        Kode
                    </th>
                    <th class="px-4 sm:px-6 py-3 sm:py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        Nama
                    </th>
                    <th class="px-4 sm:px-6 py-3 sm:py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden md:table-cell">
                        Jenis
                    </th>
                    <th class="px-4 sm:px-6 py-3 sm:py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden lg:table-cell">
                        Nilai
                    </th>
                    <th class="px-4 sm:px-6 py-3 sm:py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        Status
                    ...
                    <tr class="hover:bg-gray-50 transition-colors duration-200">
                        ...
                        <td class="px-4 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex flex-col sm:flex-row gap-2 sm:space-x-2">
                                ...
                                <a href="{{ route('admin.vouchers.show', $voucher) }}" class="bg-blue-600 hover:bg-blue-700 text-white px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-medium transition-colors duration-200 text-center">
                                    Detail
                                </a>
                                
                                <!-- ANCHOR: Edit Button -->
                                <a href="{{ route('admin.vouchers.edit', $voucher) }}" class="bg-yellow-600 hover:bg-yellow-700 text-white px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-medium transition-colors duration-200 text-center">
                                    Edit
                                </a>
                                ...
                                @if($voucher->hasBeenUsed())
                                    ...
                                    <span class="bg-gray-400 text-white px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-medium cursor-not-allowed text-center" title="Tidak dapat dihapus karena sudah digunakan">
                                        Hapus
                                    </span>
                                @else
                                    ...
                                    <form action="{{ route('admin.vouchers.destroy', $voucher) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus voucher ini?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="bg-red-600 hover:bg-red-700 text-white px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-medium transition-colors duration-200 w-full sm:w-auto">
                                            Hapus
                                        </button>
                                    </form>
                                @endif
