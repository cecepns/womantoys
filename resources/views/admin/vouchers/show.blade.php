    ...
    <div class="mb-6 {{ $config['bg'] }} border {{ $config['border'] }} rounded-lg p-4">
        <div class="flex items-center">
            <div class="w-8 h-8 sm:w-10 sm:h-10 {{ $config['icon_bg'] }} rounded-lg flex items-center justify-center mr-3">
                <svg class="w-4 h-4 sm:w-5 sm:h-5 {{ $config['icon_color'] }}" fill="none" stroke="currentColor"
                    viewBox="0 0 24 24">
            ...
            <div>
                <h3 class="text-base sm:text-lg font-semibold {{ $config['text_color'] }}">Status: {{ $config['label'] }}
                </h3>
                <p class="{{ $config['subtitle_color'] }} text-sm">Voucher dibuat pada
                    {{ $voucher->created_at->format('d F Y') }} pukul {{ $voucher->created_at->format('H:i') }} WIB</p>
            </div>
        </div>
    </div>
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-lg shadow-md border border-gray-200 p-4 md:p-6">
                <h2 class="text-lg md:text-xl font-semibold text-gray-800 mb-4 flex items-center">
                    <svg class="w-4 h-4 sm:w-5 sm:h-5 mr-2 text-pink-600" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div>
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <span
                                    class="bg-blue-100 text-blue-800 text-xs font-medium px-2.5 py-0.5 rounded">{{ $voucher->type_label }}</span>
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Nilai Diskon</label>
                                <p class="text-gray-900 font-medium text-sm sm:text-base">{{ $voucher->formatted_value }}
                                </p>
                            </div>
                    ...
                    <div>
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <p class="text-gray-900 text-sm sm:text-base">
                                    {{ $voucher->created_at->format('d F Y H:i') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                @if ($voucher->expires_at && $voucher->expires_at->lt(now()))
                    <div class="mt-6 pt-6 border-t border-gray-200">
                        <div class="bg-red-50 border-l-4 border-red-500 p-4 rounded">
                            <div class="flex items-center">
                                <svg class="w-4 h-4 sm:w-5 sm:h-5 text-red-500 mr-2 flex-shrink-0" fill="none"
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-red-700 mt-1">Voucher ini sudah kadaluarsa pada
                                        {{ $voucher->expires_at->format('d F Y H:i') }}. Voucher tidak dapat digunakan lagi
                                        oleh pelanggan.</p>
                                </div>
                            </div>
                        </div>
                ...
                @if ($voucher->usage_limit && $voucher->getUsageCount() >= $voucher->usage_limit)
                    <div class="mt-6 pt-6 border-t border-gray-200">
                        <div class="bg-yellow-50 border-l-4 border-yellow-500 p-4 rounded">
                            <div class="flex items-center">
                                <svg class="w-4 h-4 sm:w-5 sm:h-5 text-yellow-500 mr-2 flex-shrink-0" fill="none"
                                    stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.732 16.5c-.77.833.192 2.5 1.732 2.5z">
                                    </path>
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-yellow-700 mt-1">Voucher ini sudah mencapai batas maksimal
                                        penggunaan ({{ $voucher->usage_limit }} kali). Voucher tidak dapat digunakan lagi
                                        oleh pelanggan.</p>
                                </div>
                            </div>
                        </div>
                            ...
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th
                                        ...
                                        Order</th>
                                    <th
                                        class="px-3 sm:px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden md:table-cell">
                                        Email Customer</th>
                                    <th
                                        class="px-3 sm:px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                        Diskon</th>
                                ...
                                @foreach ($voucher->voucherUsages as $usage)
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-3 sm:px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                                            {{ $usage->used_at->format('d/m/Y H:i') }}</td>
                                        ...
                                        <td
                                            class="px-3 sm:px-4 py-4 whitespace-nowrap text-sm text-gray-900 hidden md:table-cell">
                                            {{ $usage->customer_email }}</td>
                                        <td class="px-3 sm:px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                                            {{ $usage->formatted_discount_amount }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
        ...
        <div class="space-y-6">
                ...
                <div class="space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="text-center p-3 sm:p-4 bg-green-50 rounded-lg">
                            <h4 class="text-xl sm:text-2xl font-bold text-green-600">{{ $statistics['remaining_usage'] }}
                            </h4>
            ...
            <div class="bg-white rounded-lg shadow-md border border-gray-200 p-4 md:p-6">
                ...
                <div class="border border-gray-200 rounded-lg p-4 bg-gray-50 text-center">
                    ...
                    <div class="mb-3">
                        <span
                            class="bg-green-100 text-green-800 text-xs sm:text-sm font-medium px-3 py-1 rounded-full">{{ $voucher->formatted_value }}</span>
                    </div>
                    <div>
