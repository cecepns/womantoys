...
<div class="mb-6 md:mb-8">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl md:text-3xl font-bold text-gray-800">Detail Voucher: {{ $voucher->code }}</h1>
            <p class="text-gray-600 mt-1 md:mt-2 text-sm md:text-base">Lihat detail lengkap voucher</p>
        </div>
...
<div class="mb-6 {{ $config['bg'] }} border {{ $config['border'] }} rounded-lg p-4">
    <div class="flex items-center">
        <div class="w-8 h-8 sm:w-10 sm:h-10 {{ $config['icon_bg'] }} rounded-lg flex items-center justify-center mr-3">
            <svg class="w-4 h-4 sm:w-5 sm:h-5 {{ $config['icon_color'] }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
            </svg>
        </div>
        <div>
            <h3 class="text-base sm:text-lg font-semibold {{ $config['text_color'] }}">Status: {{ $config['label'] }}</h3>
            <p class="{{ $config['subtitle_color'] }} text-sm">Voucher dibuat pada {{ $voucher->created_at->format('d F Y') }} pukul {{ $voucher->created_at->format('H:i') }} WIB</p>
        </div>
    </div>
</div>
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <div class="bg-white rounded-lg shadow-md border border-gray-200 p-4 md:p-6">
                ...
                <div>
                    <div class="space-y-4">
                        <div>
                            ...
                            <p class="text-gray-900 font-medium text-sm sm:text-base">{{ $voucher->code }}</p>
                        </div>
                        
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Nama Voucher</label>
                            <p class="text-gray-900 text-sm sm:text-base">{{ $voucher->name }}</p>
                        </div>
                        ...
                        <div>
                            ...
                            <p class="text-gray-900 font-medium text-sm sm:text-base">{{ $voucher->formatted_value }}</p>
                        </div>
                        ...
                        <div>
                            ...
                            <p class="text-gray-900 text-sm sm:text-base">Rp {{ number_format($voucher->max_discount, 0, ',', '.') }}</p>
                        </div>
                        @endif
                    </div>
                </div>
                ...
                <div>
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <p class="text-gray-900 text-sm sm:text-base">{{ $voucher->getUsageCount() }} kali</p>
                        </div>
                        ...
                        <div>
                            ...
                            <p class="text-gray-900 text-sm sm:text-base">{{ $voucher->created_at->format('d F Y H:i') }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-6 pt-6 border-t border-gray-200">
                ...
                <p class="text-gray-900 text-sm sm:text-base">{{ $voucher->description }}</p>
            </div>
            @endif
            ...
            <div class="mt-6 pt-6 border-t border-gray-200">
                <div class="bg-yellow-50 border-l-4 border-yellow-500 p-4 rounded">
                    <div class="flex items-center">
                        <svg class="w-4 h-4 sm:w-5 sm:h-5 text-yellow-500 mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.732 16.5c-.77.833.192 2.5 1.732 2.5z"></path>
                        </svg>
                        <div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-3 sm:px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tanggal</th>
                            <th class="px-3 sm:px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Order</th>
                            <th class="px-3 sm:px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden md:table-cell">Email Customer</th>
                            <th class="px-3 sm:px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Diskon</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach($voucher->voucherUsages as $usage)
                        <tr class="hover:bg-gray-50">
                            <td class="px-3 sm:px-4 py-4 whitespace-nowrap text-sm text-gray-900">{{ $usage->used_at->format('d/m/Y H:i') }}</td>
                            ...
                            <td class="px-3 sm:px-4 py-4 whitespace-nowrap text-sm text-gray-900 hidden md:table-cell">{{ $usage->customer_email }}</td>
                            <td class="px-3 sm:px-4 py-4 whitespace-nowrap text-sm text-gray-900">{{ $usage->formatted_discount_amount }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
    ...
    <div class="space-y-6">
            ...
            <div class="space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div class="text-center p-3 sm:p-4 bg-blue-50 rounded-lg">
                        <h4 class="text-xl sm:text-2xl font-bold text-blue-600">{{ $statistics['total_used'] }}</h4>
                        <small class="text-gray-500 text-xs">Total Digunakan</small>
                    </div>
                    <div class="text-center p-3 sm:p-4 bg-green-50 rounded-lg">
                        <h4 class="text-xl sm:text-2xl font-bold text-green-600">{{ $statistics['remaining_usage'] }}</h4>
                        <small class="text-gray-500 text-xs">Sisa Kuota</small>
                    </div>
                </div>
                
                <div class="text-center p-3 sm:p-4 bg-pink-50 rounded-lg">
                    <h5 class="text-lg sm:text-xl font-bold text-pink-600">{{ $statistics['total_discount_given'] ? 'Rp ' . number_format($statistics['total_discount_given'], 0, ',', '.') : 'Rp 0' }}</h5>
        ...
        <div class="bg-white rounded-lg shadow-md border border-gray-200 p-4 md:p-6">
            ...
            <div class="border border-gray-200 rounded-lg p-4 bg-gray-50 text-center">
                ...
                <h5 class="font-bold text-base sm:text-lg uppercase text-gray-800 mb-2">{{ $voucher->code }}</h5>
                <p class="text-gray-700 text-sm mb-3">{{ $voucher->name }}</p>
                @if($voucher->description)
                <p class="text-gray-500 text-xs mb-3">{{ Str::limit($voucher->description, 50) }}</p>
                @endif
                <div class="mb-3">
                    <span class="bg-green-100 text-green-800 text-xs sm:text-sm font-medium px-3 py-1 rounded-full">{{ $voucher->formatted_value }}</span>
                </div>
                <div>
