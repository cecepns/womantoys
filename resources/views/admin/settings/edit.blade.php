...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <form method="POST" action="{{ route('admin.settings.password') }}">
                ...
                <div class="space-y-4 sm:space-y-6">
                    <div>
                        ...
                        <div class="relative">
                            <input id="current_password" type="password" name="current_password" class="w-full px-3 sm:px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500 pr-10 text-sm" required>
                            <button type="button" class="absolute inset-y-0 right-2 flex items-center text-gray-500 hover:text-gray-700" onclick="togglePassword('current_password', this)" aria-label="Tampilkan/Sembunyikan kata sandi">
                                <svg class="icon-eye w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.477 0 8.268 2.943 9.542 7-1.274 4.057-5.065 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                </svg>
                                <svg class="icon-eye-off w-4 h-4 sm:w-5 sm:h-5 hidden" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input id="new_password" type="password" name="new_password" class="w-full px-3 sm:px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500 pr-10 text-sm" required>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input id="new_password_confirmation" type="password" name="new_password_confirmation" class="w-full px-3 sm:px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500 pr-10 text-sm" required>
                    ...
                    <div class="pt-2">
                        <button type="submit" class="w-full sm:w-auto bg-pink-600 hover:bg-pink-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors duration-200">Simpan</button>
                    </div>
                </div>
            </form>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-md border border-gray-200">
        <div class="p-4 sm:p-6 border-b border-gray-200">
            <h3 class="text-base sm:text-lg font-semibold text-gray-800">Alamat Toko</h3>
            <p class="text-xs sm:text-sm text-gray-600 mt-1">Alamat ini dipakai sebagai origin di checkout.</p>
        </div>
        <div class="p-4 sm:p-6">
            <form method="POST" action="{{ route('admin.settings.store') }}" id="store-setting-form">
                ...
                <div class="space-y-4 sm:space-y-6">
                    <div>
                        ...
                        <input type="text" name="store_name" value="{{ old('store_name', $storeName) }}" class="w-full px-3 sm:px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500 text-sm">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Kota/Kabupaten (Origin)</label>
                        <div class="relative">
                            <input type="text" id="city-autocomplete" value="{{ old('store_city_label', $storeCityLabel) }}" placeholder="Ketik kota/kabupaten..." autocomplete="off" class="w-full px-3 sm:px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500 text-sm">
                    ...
                    <div>
                        ...
                        <textarea name="store_address" rows="3" class="w-full px-3 sm:px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500 text-sm" placeholder="Nama jalan, nomor, RT/RW, dsb">{{ old('store_address', $storeAddress) }}</textarea>
                    </div>
                    <div class="pt-2">
                        <button type="submit" class="w-full sm:w-auto bg-pink-600 hover:bg-pink-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors duration-200">Simpan</button>
                    </div>
    ...
    <div class="bg-white rounded-lg shadow-md border border-gray-200">
        <div class="p-4 sm:p-6 border-b border-gray-200">
            ...
            <p class="text-xs sm:text-sm text-gray-600 mt-1">Atur nomor WhatsApp dan pesan default untuk floating button.</p>
        </div>
        <div class="p-4 sm:p-6">
            <form method="POST" action="{{ route('admin.settings.whatsapp') }}" id="whatsapp-setting-form">
                ...
                <div class="space-y-4 sm:space-y-6">
                    <div>
                        ...
                        <div class="flex">
                            <span class="inline-flex items-center justify-center px-3 py-2 sm:py-2 border border-gray-300 bg-gray-50 text-gray-500 text-sm rounded-l-lg">
                                +62
                            </span>
                            <input type="text" name="whatsapp_number" value="{{ old('whatsapp_number', $whatsappNumber) }}" placeholder="81234567890" class="flex-1 px-3 sm:px-4 py-2 border border-gray-300 rounded-r-lg border-l-0 focus:outline-none focus:ring-2 focus:ring-pink-500 text-sm" pattern="[0-9]+" title="Masukkan hanya angka">
                        </div>
                    ...
                    <div>
                        ...
                        <textarea name="whatsapp_message" rows="3" class="w-full px-3 sm:px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500 text-sm" placeholder="Pesan yang akan dikirim saat user klik floating button">{{ old('whatsapp_message', $whatsappMessage) }}</textarea>
                        <p class="text-xs text-gray-500 mt-1">Pesan ini akan otomatis terisi saat user mengklik floating button WhatsApp</p>
                        @error('whatsapp_message')
                            <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="pt-2">
                        <button type="submit" class="w-full sm:w-auto bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors duration-200">Simpan</button>
                    </div>
                </div>
            </form>
        </div>
