        ...
        <form method="GET" action="{{ route('admin.products.index') }}" class="flex flex-col md:flex-row gap-4">
            ...
            <div class="md:w-48">
                <select name="status"
                    ...
                    <option value="out_of_stock" {{ request('status') == 'out_of_stock' ? 'selected' : '' }}>Stok Habis
                    </option>
                </select>
            </div>
            <div class="md:w-48">
            ...
            @if (request('search') || request('category') || request('status') || request('featured'))
                <a href="{{ route('admin.products.index') }}"
                    class="bg-red-600 hover:bg-red-700 text-white py-2 px-4 rounded-lg transition-colors duration-200 flex items-center">
                    Reset
                </a>
            @endif
        </form>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($products as $product)
                        <tr class="hover:bg-gray-50">
                            ...
                            @else bg-purple-100 text-purple-800 @endif rounded-full">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    class="px-2 py-1 text-xs font-medium {{ $product->status_badge_class }} rounded-full">
                                    {{ $product->status_label }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    class="px-2 py-1 text-xs font-medium {{ $product->featured_status_badge_class }} rounded-full">
