...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
    ...
    <div class="bg-white rounded-lg shadow-md p-6 border border-gray-200">
        ...
        <div class="mt-4">
            <span class="text-sm {{ $orderGrowth >= 0 ? 'text-green-600' : 'text-red-600' }} font-medium">{{ $orderGrowth >= 0 ? '+' : '' }}{{ $orderGrowth }}%</span>
    ...
    <div class="bg-white rounded-lg shadow-md p-6 border border-gray-200">
        ...
        <div class="mt-4">
            <span class="text-sm text-green-600 font-medium">+{{ $newProductsThisMonth }}</span>
            <span class="text-sm text-gray-600">produk baru bulan ini</span>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-md p-6 border border-gray-200">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-3xl font-bold text-gray-800">{{ $formattedTotalRevenue }}</p>
            </div>
        ...
        <div class="mt-4">
            <span class="text-sm {{ $revenueGrowth >= 0 ? 'text-green-600' : 'text-red-600' }} font-medium">{{ $revenueGrowth >= 0 ? '+' : '' }}{{ $revenueGrowth }}%</span>
            <span class="text-sm text-gray-600">dari bulan lalu</span>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-md p-6 border border-gray-200">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-3xl font-bold text-gray-800">{{ number_format($pendingOrders) }}</p>
            </div>
        ...
        <div class="mt-4">
            <span class="text-sm {{ $pendingChange >= 0 ? 'text-red-600' : 'text-green-600' }} font-medium">{{ $pendingChange >= 0 ? '+' : '' }}{{ $pendingChange }}</span>
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
    ...
    <div class="bg-white rounded-lg shadow-md p-6 border border-gray-200">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-sm font-medium text-gray-600">Pendapatan Minggu Ini</p>
                <p class="text-2xl font-bold text-gray-800">{{ 'Rp ' . number_format($thisWeekRevenue, 0, ',', '.') }}</p>
            </div>
    ...
    <div class="bg-white rounded-lg shadow-md p-6 border border-gray-200">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-sm font-medium text-gray-600">Pendapatan Pending</p>
                <p class="text-2xl font-bold text-gray-800">{{ 'Rp ' . number_format($pendingRevenue, 0, ',', '.') }}</p>
            </div>
...
<div class="bg-white rounded-lg shadow-md border border-gray-200 mb-8">
    <div class="p-6 border-b border-gray-200">
        <h3 class="text-lg font-semibold text-gray-800">Breakdown Status Pesanan</h3>
        <p class="text-sm text-gray-600">Distribusi pesanan berdasarkan status</p>
    </div>
    <div class="p-6">
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            <!-- Pending Payment -->
            <div class="text-center p-4 bg-yellow-50 rounded-lg border border-yellow-200">
                <div class="text-2xl font-bold text-yellow-700">{{ $orderStatusBreakdown['pending_payment'] }}</div>
                <div class="text-sm text-yellow-600 font-medium">Menunggu Pembayaran</div>
            </div>
            
            <!-- Paid -->
            <div class="text-center p-4 bg-green-50 rounded-lg border border-green-200">
                <div class="text-2xl font-bold text-green-700">{{ $orderStatusBreakdown['paid'] }}</div>
                <div class="text-sm text-green-600 font-medium">Sudah Dibayar</div>
            </div>
            
            <!-- Processing -->
            <div class="text-center p-4 bg-blue-50 rounded-lg border border-blue-200">
                <div class="text-2xl font-bold text-blue-700">{{ $orderStatusBreakdown['processing'] }}</div>
                <div class="text-sm text-blue-600 font-medium">Sedang Diproses</div>
            </div>
            
            <!-- Shipped -->
            <div class="text-center p-4 bg-purple-50 rounded-lg border border-purple-200">
                <div class="text-2xl font-bold text-purple-700">{{ $orderStatusBreakdown['shipped'] }}</div>
                <div class="text-sm text-purple-600 font-medium">Dikirim</div>
            </div>
            
            <!-- Delivered -->
            <div class="text-center p-4 bg-gray-50 rounded-lg border border-gray-200">
                <div class="text-2xl font-bold text-gray-700">{{ $orderStatusBreakdown['delivered'] }}</div>
                <div class="text-sm text-gray-600 font-medium">Diterima</div>
            </div>
            
            <!-- Cancelled -->
            <div class="text-center p-4 bg-red-50 rounded-lg border border-red-200">
                <div class="text-2xl font-bold text-red-700">{{ $orderStatusBreakdown['cancelled'] }}</div>
                <div class="text-sm text-red-600 font-medium">Dibatalkan</div>
            </div>
        </div>
    </div>
</div>
                ...
                @forelse($recentOrders as $order)
                    <div class="flex items-center justify-between">
                        ...
                        <div class="text-right">
                            <p class="font-medium text-gray-800">{{ $order->formatted_total_amount }}</p>
                            @php
                            ...
                            @endphp
                            <span class="px-2 py-1 text-xs {{ $statusConfig['class'] }} rounded-full">{{ $statusConfig['text'] }}</span>
                        </div>
                    </div>
                @empty
