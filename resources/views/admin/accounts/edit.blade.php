...
@section('title', 'Edit Rekening Bank - Admin Panel')
...
@section('content')
    ...
    <div class="flex justify-between items-center mb-6 flex-col sm:flex-row gap-4 sm:gap-0">
        <div>
            ...
            <p class="text-gray-600">Edit rekening: {{ $account->bank_name }}</p>
        </div>
        <a href="{{ route('admin.accounts.index') }}"
            class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg font-medium transition-colors duration-200 w-full sm:w-auto text-center">
            ...
            <form action="{{ route('admin.accounts.update', $account) }}" method="POST">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" id="bank_name" name="bank_name"
                        ...
                        placeholder="Contoh: Bank Central Asia (BCA)" required>
                    @error('bank_name')
                ...
                <!-- ANCHOR: Atas Nama (Pemilik Rekening) -->
                <div class="mb-6">
                    ...
                    <input type="text" id="account_holder" name="account_holder"
                        ...
                        placeholder="Contoh: PT. Woman Toys Indonesia" required>
                ...
                <div class="mb-6">
                    ...
                    <select id="status" name="status"
                        ...
                        <option value="active"
                            {{ old('status', $account->is_active ? 'active' : 'inactive') === 'active' ? 'selected' : '' }}>
                            Aktif</option>
                        <option value="inactive"
                            {{ old('status', $account->is_active ? 'active' : 'inactive') === 'inactive' ? 'selected' : '' }}>
                            Tidak Aktif</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                    <p class="mt-1 text-sm text-gray-500">Pilih status rekening untuk menentukan apakah dapat digunakan
                        untuk pembayaran.</p>
                </div>

                <!-- ANCHOR: Tombol Submit -->
                <div class="flex justify-end flex-col sm:flex-row gap-2 sm:gap-3">
                    <a href="{{ route('admin.accounts.index') }}"
                        class="bg-gray-500 hover:bg-gray-600 text-white px-6 py-2 rounded-lg font-medium transition-colors duration-200 w-full sm:w-auto text-center">
                        Batal
                    </a>
                    <button type="submit"
                        class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg font-medium transition-colors duration-200 w-full sm:w-auto">
                        <svg class="w-5 h-5 inline mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
