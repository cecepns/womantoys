    ...
    <div class="mb-8">
        <div class="flex items-center justify-between flex-col sm:flex-row gap-4 sm:gap-0">
            ...
            <a href="{{ route('admin.orders.index') }}"
                class="bg-gray-600 hover:bg-gray-700 text-white font-medium py-2 px-4 rounded-lg transition-colors duration-200 flex items-center w-full sm:w-auto justify-center">
    ...
    <div class="mb-6 {{ $config['bg'] }} border {{ $config['border'] }} rounded-lg p-4">
        <div class="flex items-center">
            ...
            <div>
                ...
                <p class="{{ $config['subtitle_color'] }}">Pesanan dibuat pada {{ $order->created_at->format('d F Y') }}
                    pukul {{ $order->created_at->format('H:i') }} WIB</p>
            </div>
        </div>
    </div>
            ...
            @if ($order->voucher_id || $order->voucher_code)
                <div class="bg-white rounded-lg shadow-md border border-gray-200 p-6">
                    <h2 class="text-xl font-semibold text-gray-800 mb-4 flex items-center">
                        <svg class="w-5 h-5 mr-2 text-pink-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 00 2 2h3l2 2h6l2-2h3a2 2 0 002-2V7a2 2 0 00-2-2H5z">
                            </path>
                        </svg>
                        Informasi Voucher
                    </h2>
        ...
        <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <select id="status" name="status"
                            ...
                            <option value="menunggu_pembayaran"
                                {{ $order->status === 'menunggu_pembayaran' ? 'selected' : '' }}>
                                Menunggu Pembayaran
                            </option>
            ...
            @if ($order->payment_proof_path)
                <div class="bg-white rounded-lg shadow-md border border-gray-200 p-6">
                    ...
                    <div class="space-y-3">
                        <div>
                            <img src="{{ asset('storage/' . $order->payment_proof_path) }}" alt="Bukti Transfer"
