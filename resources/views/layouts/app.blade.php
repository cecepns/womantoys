...
<body class="min-h-screen bg-gray-50">
    ...
    <!-- ANCHOR: Tombol Chat WhatsApp -->
    <div class="fixed bottom-6 right-6 z-50">
        <button
            class="fab w-14 h-14 bg-green-500 hover:bg-green-600 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 flex items-center justify-center group"
            onclick="openChat()" aria-label="Chat dengan kami">
        ...
        <div class="container mx-auto px-4 py-6 md:py-8">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                ...
                <div>
                    ...
                    <ul class="space-y-2">
                        <li><a href="#"
                                class="text-gray-300 hover:text-white transition-colors duration-200 text-sm md:text-base">Cara
                                Memesan</a></li>
                        <li><a href="#"
                                class="text-gray-300 hover:text-white transition-colors duration-200 text-sm md:text-base">Kebijakan
                                Privasi</a></li>
                        <li><a href="#"
                                class="text-gray-300 hover:text-white transition-colors duration-200 text-sm md:text-base">Syarat
                                & Ketentuan</a></li>
                    </ul>
                </div>
            ...
            <div class="border-t border-gray-700 mt-6 md:mt-8 pt-6 md:pt-8 text-center">
                <p class="text-gray-300 text-sm md:text-base">&copy; 2024 {{ $storeName ?? 'WomanToys' }}. Semua hak
                    dilindungi.</p>
            </div>
        </div>
    </footer>
    ...
    <!-- ANCHOR: Owl Carousel JS CDN -->
    <script src="https://cdnjs.cloudflare.com/ajax/libs/OwlCarousel2/2.3.4/owl.carousel.min.js"></script>

    <!-- ANCHOR: Fallback jQuery jika belum ter-load -->
    <script>
    ...
    <!-- ANCHOR: Inisialisasi Script Halaman -->
    <script>
        // ANCHOR: Pastikan semua script ter-load sebelum inisialisasi
        window.addEventListener('load', function() {
            // Memicu event kustom untuk script spesifik halaman
            if (typeof window.initPageScripts === 'function') {
                window.initPageScripts();
            }
        });

        // ANCHOR: Fungsionalitas Menu Mobile
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (mobileMenuBtn) {
                mobileMenuBtn.addEventListener('click', function() {
                    ...
                    if (isHidden) {
                        ...
                        // Sekaligus tutup submenu yang sedang terbuka saat close
            ...
            // ANCHOR: Tutup menu kategori saat klik di luar (mobile)
            ...
            // ANCHOR: Tutup menu dengan tombol Escape (mobile)
            ...
            // ANCHOR: Peningkatan fungsi pencarian
            const searchInput = document.querySelector('input[name="search"]');
            const searchForm = document.querySelector('form[action="/catalog"]');

            if (searchInput && searchForm) {
                // Otomatis submit setelah 1 detik tanpa ketikan
                let searchTimeout;
            ...
            // ANCHOR: Smooth scrolling navigasi kategori (placeholder)
            const categoryLinks = document.querySelectorAll('a[href*="category"]');
            categoryLinks.forEach(link => {
                link.addEventListener('click', function(e) {
                    // Tambahkan perilaku smooth scroll jika diperlukan
                });
            });

            // ANCHOR: Sembunyikan item menu berlebih pada layar lebar
            ...
            function adjustCategoriesLayout() {
                ...
                // Reset semua item agar terlihat
                items.forEach(item => {
                    item.style.display = '';
                });
                // Hanya terapkan pada breakpoint lg ke atas (>=1024px)
            ...
            // Jalankan saat load dan resize (dengan debounce)
            ...
            // ANCHOR: Fungsionalitas Floating Action Button (WhatsApp)
            window.openChat = function() {
                // Ambil pengaturan WhatsApp dari PHP
                const whatsappPhone = '{{ $whatsappNumber }}';
                const whatsappText = encodeURIComponent(
                    '{{ $whatsappMessage }}');
                const whatsappUrl =
                    `https://api.whatsapp.com/send/?phone=62${whatsappPhone}&text=${whatsappText}&type=phone_number&app_absent=0`;
