...
<div class="container mx-auto px-4 py-6 md:py-10">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8 lg:gap-12 mb-12 md:mb-16">
        ...
        <div>
            ...
            <div class="mb-4 md:mb-6">
                @if($product->main_image_url)
                    ...
                    >
                @else
                    <div class="w-full h-64 sm:h-80 md:h-96 lg:h-[500px] bg-gray-200 flex items-center justify-center rounded-lg shadow-lg">
                        <div class="text-center">
                            <svg class="mx-auto h-16 w-16 md:h-24 md:w-24 text-gray-400 mb-2 md:mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <!-- Thumbnail Gallery -->
            <div class="flex overflow-x-auto space-x-3 md:space-x-4 py-2 scrollbar-hide" style="scrollbar-width: none; -ms-overflow-style: none;">
                @if($product->main_image_url)
                    <img 
                        src="{{ $product->main_image_url }}" 
        ...
        <div>
            ...
            <!-- Product Price -->
            <p class="text-2xl md:text-3xl font-bold text-pink-600 mb-4 md:mb-6">
            ...
            @if($product->stock > 0)
                <a href="{{ route('checkout') }}?product={{ $product->id }}" class="block">
                    <button class="w-full bg-pink-600 hover:bg-pink-700 text-white font-bold py-3 md:py-4 px-6 md:px-8 rounded-lg text-lg md:text-xl transition-colors duration-200 shadow-lg hover:shadow-xl">
                        Beli Sekarang
                    </button>
                </a>
            @else
                <button class="w-full bg-gray-400 text-white font-bold py-3 md:py-4 px-6 md:px-8 rounded-lg text-lg md:text-xl cursor-not-allowed" disabled>
                    Habis Stok
                </button>
            @endif
        </div>
    ...
    <div class="pt-6 md:pt-8">
        ...
        <div class="flex flex-col sm:flex-row border-b border-gray-200 mb-6 md:mb-8">
            <button class="px-3 w-full  sm:w-auto md:px-6 py-2 md:py-3 text-gray-500 hover:text-gray-700 font-medium rounded-t-lg transition-all duration-200 text-sm md:text-base" data-tab="description" id="tab-description">
                Deskripsi Lengkap
            </button>
            <button class="px-3 w-full sm:w-auto md:px-6 py-2 md:py-3 text-gray-500 hover:text-gray-700 font-medium rounded-t-lg transition-all duration-200 text-sm md:text-base" data-tab="specifications" id="tab-specifications">
                Spesifikasi
            </button>
            <button class="px-3 w-full sm:w-auto md:px-6 py-2 md:py-3 text-gray-500 hover:text-gray-700 font-medium rounded-t-lg transition-all duration-200 text-sm md:text-base" data-tab="care" id="tab-care">
                Petunjuk Perawatan
            </button>
        </div>
