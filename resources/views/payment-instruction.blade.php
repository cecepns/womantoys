            ...
            <div class="bg-white border border-gray-200 rounded-lg p-4 md:p-6 mb-6 md:mb-8">
                ...
                <div class="space-y-2 md:space-y-3">
                    <div class="flex flex-col sm:flex-row sm:justify-between gap-1 sm:gap-0">
                        ...
                        <span
                            class="font-semibold text-sm md:text-base text-gray-800 break-all">{{ $order->order_number }}</span>
                    </div>
                    <div class="flex flex-col sm:flex-row sm:justify-between gap-1 sm:gap-0">
                        <span class="text-sm md:text-base text-gray-600">Total Pembayaran:</span>
                        <span
                            class="text-xl md:text-2xl font-bold text-pink-600">{{ $order->formatted_total_amount }}</span>
                    </div>
                    <div class="flex flex-col sm:flex-row sm:justify-between gap-1 sm:gap-0">
                        <span class="text-sm md:text-base text-gray-600">Tanggal Pesanan:</span>
                        <span
                            class="font-semibold text-sm md:text-base text-gray-800">{{ $order->created_at->format('d F Y') }}</span>
                    </div>
                    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 sm:gap-0">
                        ...
                        @if ($order->isPendingPayment())
                            <span
                                class="px-2 md:px-3 py-1 bg-yellow-100 text-yellow-800 rounded-full text-xs md:text-sm font-medium self-start sm:self-auto">Menunggu
                                Pembayaran</span>
                        @elseif($order->isPaid())
                            <span
                                class="px-2 md:px-3 py-1 bg-green-100 text-green-800 rounded-full text-xs md:text-sm font-medium self-start sm:self-auto">Sudah
                                Dibayar</span>
                        @elseif($order->isProcessing())
                            <span
                                class="px-2 md:px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-xs md:text-sm font-medium self-start sm:self-auto">Sedang
                                Diproses</span>
                        @elseif($order->isShipped())
                            <span
                                class="px-2 md:px-3 py-1 bg-purple-100 text-purple-800 rounded-full text-xs md:text-sm font-medium self-start sm:self-auto">Dikirim</span>
                        @elseif($order->isDelivered())
                            <span
                                class="px-2 md:px-3 py-1 bg-green-100 text-green-800 rounded-full text-xs md:text-sm font-medium self-start sm:self-auto">Diterima</span>
                        @elseif($order->isCancelled())
                            <span
                                class="px-2 md:px-3 py-1 bg-red-100 text-red-800 rounded-full text-xs md:text-sm font-medium self-start sm:self-auto">Dibatalkan</span>
                        @endif
                    </div>
                </div>
            </div>
                            ...
                            <div class="hidden sm:flex justify-between items-center">
                                <div>
                                    ...
                                    <div class="flex items-center gap-4 mt-1">
                                        ...
                                        @if ($item->product && $item->product->weight)
                                            <p class="text-sm text-gray-600">Berat: {{ $item->product->formatted_weight }}
                                                / pcs</p>
                                        @endif
                                    </div>
                                </div>
            ...
            <div class="bg-white border border-gray-200 rounded-lg p-4 md:p-6 mb-6 md:mb-8">
                ...
                <div class="space-y-2 md:space-y-3">
                    <div>
                        ...
                        <span
                            class="font-medium text-sm md:text-base text-gray-800 whitespace-pre-line break-words">{{ $order->shipping_address }}</span>
                    </div>
                    <div class="flex flex-col sm:flex-row sm:justify-between gap-1 sm:gap-0">
                    ...
                    <div class="flex flex-col sm:flex-row sm:justify-between gap-1 sm:gap-0">
                        ...
                        <span
                            class="font-semibold text-sm md:text-base text-gray-800">{{ $order->formatted_shipping_cost }}</span>
                    </div>
                </div>
            </div>
                    ...
                    @forelse($activeAccounts as $account)
                        <div class="p-3 md:p-4 bg-gray-50 rounded-lg">
                            ...
                            <div class="block sm:hidden">
                                ...
                                <p class="font-mono font-semibold text-gray-800 text-base mb-1 break-all">
                            ...
                            <div class="hidden sm:flex justify-between items-center">
                                ...
                                <div class="text-right">
                                    <p class="font-mono font-semibold text-gray-800 text-lg">
                                        {{ $account->account_number }}</p>
                ...
                @else
                    <form action="{{ route('payment.confirm') }}" method="POST" enctype="multipart/form-data"
                        ...
                        <div>
                            ...
                            <div id="upload-area"
                                class="border-2 border-dashed border-gray-300 rounded-lg p-4 md:p-6 text-center hover:border-pink-400 transition-colors duration-200">
                                <svg class="w-8 h-8 md:w-12 md:h-12 text-gray-400 mx-auto mb-3 md:mb-4" fill="none"
                                    stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12">
                                    </path>
                                </svg>
                                <p class="text-sm md:text-base text-gray-600 mb-2">Klik untuk mengunggah atau seret dan
                                    lepas</p>
                            ...
                            <div id="preview-area" class="hidden">
                                ...
                                <div id="image-preview" class="hidden">
                                    ...
                                    <div class="mt-2 md:mt-3 text-center">
                                        ...
                                        <p class="text-xs text-gray-500 mt-1">Hapus file terlebih dahulu untuk mengupload
                                            file baru</p>
                                    </div>
                                </div>
                                ...
                                <div id="pdf-preview" class="hidden">
                                    <div
                                        class="flex items-center justify-center p-4 md:p-8 border border-gray-200 rounded-lg bg-gray-50">
                                        <div class="text-center">
                                            <svg class="w-12 h-12 md:w-16 md:h-16 text-red-500 mx-auto mb-3 md:mb-4"
                                            ...
                                            <p id="pdf-file-name" class="text-xs md:text-sm text-gray-600 mb-3 md:mb-4">
                                            </p>
                                            <button onclick="deleteFile()"
                                                ...
                                                <svg class="w-3 h-3 md:w-4 md:h-4 inline mr-1 md:mr-2" fill="none"
                                                    stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                        d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16">
                                                    </path>
                        ...
                        <button type="submit" id="submitBtn"
                            ...
                            <span id="submitLoading" class="hidden">
                                <svg class="animate-spin -ml-1 mr-2 md:mr-3 h-4 w-4 md:h-5 md:w-5 text-white inline"
